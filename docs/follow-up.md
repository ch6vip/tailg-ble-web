...
- [ ] 真机验证危险动作长按交互。
  ...
  - 真机步骤：在安卓 Chrome 和 Edge 中分别登录或连接车辆，长按“安全设防”和“切断电门”到 0.5 秒后松手，确认不会发出命令且反馈恢复；再次长按超过 1 秒，确认只发送一次命令。
  - 真机步骤：长按过程中手指滑出按钮、系统切后台、页面失焦或触发返回手势，确认 hold 状态取消、按钮文案恢复、无残留进度条。
  - 记录字段：设备型号、浏览器版本、系统版本、页面 URL、设防结果、断电结果、是否出现系统菜单或误触发。
  - 本地补充验证：2026-05-24 Chrome DevTools 合成 pointer 事件验证，短按取消后 `is-holding=false`、按钮文案恢复、反馈从 `Hold` 回到 `Idle`；超过阈值后不会残留 holding 状态。该结果不替代安卓真机发车验收。

## 中优先级

- [x] 将 `cloudMode` 重构为明确的 `activeChannel`。
...
- [x] 用 class 状态替代内联 `style="display:none"`。
...
- [x] 全部修复完成后执行统一验证，验证通过后才能上传仓库。
  ...
  - 结果：2026-05-24 已通过 `npm run build`、Chrome DevTools 360px 移动端布局检查、控制台 error/warn 检查、Lighthouse mobile snapshot 四项 100 分和 `git status` 审计。
