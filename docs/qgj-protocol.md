...
- **状态查询 + 心跳通道**（feb3）— GATT READ 返回 BikeStatusModel，登录后必须 1Hz 持续读取，否则 ~3s 设备主动断开
...
0xfeb0  ★ kuyi 认证 + 控车 + 心跳通道
        feb1 [Write, WriteNoResp]   - 写认证/控车帧
        feb2 [Indicate]              - 接收响应
        feb3 [Read]                  - ★ ECU_GET_STATUS（心跳必读，返回 BikeStatusModel）
        feb4 [Read]                  - ECU_QUERY_KEY_VERSION / ECU_QUERY_BIKE_INFO
...
**3 秒空闲超时**：登录后若 feb3 无 GATT READ 请求，设备 ~3 秒就主动断开（实测 2026-05-25）。**重发 kuyi 帧到 feb1 不算心跳**——必须 read feb3。详见下方"心跳"章节。登录前发任何指令到 fcc1 都返回 `21000000`（固件级"未认证"）。

## 心跳 / Keep-Alive（cmdID 0x1002 名义，实际是 feb3 GATT READ）

**源码级 confirmed** —— 心跳是周期性 **GATT READ feb3**，不是任何 kuyi V2 写入帧：

1. 周期调度：`com/kuyi/blesdk/model/EcuStatus.java:251` → `postDelayed(runnable, 1000L)` 登录后每 1 秒触发
2. Runnable 内容：`EcuStatus.java:47` → `manager.d(new Command.EcuGetStatus())`
3. V2 派发：`com/kuyi/h/f1.java:111-118` 命中 `b1.f25605a[Command.Tag.ECU_GET_STATUS] = 1` 分支 → `readCharacteristic(this.s).with(...)`
4. `this.s` UUID：`com/kuyi/h/e1.java:108` → `service.getCharacteristic(f1.C)`，其中 `f1.C = RadarBleConfig.GATT_UUID_feb2 = "0000feb3-..."`
5. 读回字节由 `StatusParams` 解码（电量/电压/锁状态/上电状态等），喂给 `BikeStatusModel` LiveData

**实现要点：**

- 登录 (feb2 收到 `A7 00 00 03 10 01 01`) 后立即触发首次 read，之后 setInterval 1000ms
- read 失败或状态退出 authenticated 时清掉定时器
- read 结果可以丢弃（光是发出 read 请求就足以保活），解析 StatusParams 是可选增强

如果心跳间隔放宽到 ≥ 3 秒，设备会因为空闲超时主动断开 GATT。1 秒匹配官方 APP 行为，最稳。
...
| `src/ble/connection.ts` | fcc0 服务连上后自动加载 feb0、订阅 feb2、自动发登录帧、解析 0x1001 成功即认证；登录后启动 1Hz feb3 read 心跳；`awaitQgjAck()` 让控车指令等待 0x1002 回执 |
| `src/commands.ts` | `sendBleCmd` 按 `conn.serviceType==='fcc0'` 分发到 `sendQgjCmd`，写 feb1，await ack 后更新车况状态 |
...
| `com/kuyi/h/f1.java:111-118` | V2 BleManager：ECU_GET_STATUS 走 readCharacteristic(feb3) 而非 kuyi 写帧 |
| `com/kuyi/h/e1.java:99-128` | f1 的 GATT callback，把 feb1/feb2/feb3/feb4 绑到 p/q/s/r |
| `com/kuyi/blesdk/model/EcuStatus.java:47,251` | 心跳调度：登录后每 1 秒触发一次 EcuGetStatus |
